...
$("#img_input2").on("change", function(e) {
    ...
    reader.onload = function(arg) {
        var image = new Image();
        image.src=arg.target.result+"?"+Math.random();
        if(image.width<Pwidth||image.height<Pheight){
            alert("请上传大于450*450的图片！");
        }else{

            var img = '<img class="preview" src="' + image.src + '" alt="preview"/>';
             $("#preview_box2").empty().append(img);
            for(var i=0;i<3;i++){
                for(var j=0;j<3;j++){
                    var num ="d"+ String(i*3+j+1);
                    $("#"+num).css({"background":"rgba(0, 0, 0, 0) url("+image.src+") no-repeat scroll "+(j)*(-150)+"px "+ (i)*(-150)+"px;",'background-size':'450px 450px'});
                }
            }
        }
    }
...
function move(id){
    if(!pause){
        ...
        if( target_d !== 0){
            ...
            document.getElementById("step").innerHTML=sum_step++;
